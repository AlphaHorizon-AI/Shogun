...
NO_ACTIVE_MODEL_PROVIDER_MESSAGE = (
...
async def shogun_chat(
    ...
    # AgentFlow and Flow Stack operations are never conversation-only. Force the
    # governed tool lane and use a stable category marker so small local models
    # cannot accidentally lose the workflow tools during intent pruning.
    if requires_workflow_tools(user_msg):
        mode = "mission"
        classification = {
            **classification,
            "mode": "mission",
            ...
            "matched": list(dict.fromkeys([
                *classification.get("matched", []),
                *workflow_intent_keywords(user_msg),
            ])),
        }
    ...
    try:
        ...
        skill_result = await SkillActivationService(svc.session).activate(SkillActivationRequest(
            ...
            usage_location="telegram" if visual_session_id.startswith("telegram") else "chat",
        ...
        logger.warning("Active skill selection skipped for chat: %s", exc)

    # Governed Chat — context-aware with memory retrieval, no tools\r\n    if mode == \"governed\":\r\n        from shogun.api.governed_chat import _shogun_governed_chat\r\n        return await _shogun_governed_chat(user_msg, history, svc, classification=classification) — routes to mission for now
...
def _classify_chat_mode(message: str, history: list) -> dict:
    ...
    MISSION_KEYWORDS = [
        ...
        "calendar", "schedule", "remind", "reminder board", "don't forget",
        "do not forget", "follow up", "circle back", "revisit this",
        ...
        # Cron/scheduling and native workflow orchestration
        "cron", "job", "workflow", "agentflow", "agent flow",
        "flow stack", "flowstack", "stack orchestrator", "pipeline",
        # Execution/system
...
def _resolve_provider_model_id(
    ...
    *,
    ...
    """Resolve a concrete model that belongs to the selected provider."""
    from shogun.services.model_router import is_concrete_model_id

    config = provider.config or {}
    configured = [
        *list(config.get("models") or []),
        config.get("model_id"),
        config.get("model"),
        provider.name,
    ]
    configured = [
        ...
        if is_concrete_model_id(value, provider.provider_type)
    ]
    saved_belongs_to_provider = (
        bool(saved_provider_id) and saved_provider_id == str(provider.id)
    ) or saved_model_name in configured
    if saved_belongs_to_provider and is_concrete_model_id(saved_model_name, provider.provider_type):
        return saved_model_name
    return configured[0] if configured else None


async def _record_active_skill_outcomes(
    classification: dict[str, Any] | None,
    outcome: str,
    summary: str,
) -> None:
    """Close active-skill audit records without delaying or breaking chat delivery."""
    record_ids = (classification or {}).get("_active_skill_run_ids") or []
    if not record_ids:
        return
    try:
        from shogun.db.engine import async_session_factory
        ...
        async with async_session_factory() as skill_db:
            await SkillTrajectoryService(skill_db).link_output(
                record_ids,
                output_summary=summary,
                output_type="chat_response",
                metadata={"outcome": outcome},
            )
            service = SkillActivationService(skill_db)
            for record_id in record_ids:
                await service.outcome(uuid.UUID(str(record_id)), outcome, summary)
            await skill_db.commit()
    except Exception as exc:
        logger.warning("Active skill outcome recording failed: %s", exc)


async def _link_active_skill_tool(
    classification: dict[str, Any] | None,
    ...
    try:
        ...
        async with async_session_factory() as trajectory_db:
            await SkillTrajectoryService(trajectory_db).link_tool_call(
                record_ids, tool_call_id=tool_call_id, tool_name=tool_name,
                tool_input=tool_input, tool_output=tool_output, status=status,
            )
            await trajectory_db.commit()
    except Exception as exc:
        logger.warning("Active skill tool trajectory linking failed: %s", exc)
    ...
    *,
    ...
    if not provider:
        async def _no_provider():
            yield f"data: {json.dumps({'type': 'error', 'content': NO_ACTIVE_MODEL_PROVIDER_MESSAGE})}\n\n"
            yield "data: [DONE]\n\n"
...
If the user asks for memory, tools, files, commands, browsing, messages, scheduling, agent spawning, or mission execution, explain that the request requires Governed Chat or Mission Mode and they can switch using the mode selector in the chat interface."""
    ...
    if classification.get("_skill_context"):
        system_prompt += "\n\n" + classification["_skill_context"]
    ...
    async def generate():
        ...
        await _record_active_skill_outcomes(
            classification,
            "failed" if _failed else "success",
            "Fast chat failed." if _failed else "Fast chat response delivered.",
        )
...
async def _shogun_chat_internal(
    ...
    *,
    ...
    # ── 2. Resolve primary model from settings ────────────────────
        ...
        async for db in get_db():
            ...
            try:
                ...
                if provider:
                    ...
                    _reasoning_effort = route.payload.get("selected_reasoning_effort")
                    await db.commit()
            except Exception as exc:
        ...
        if not model_name:
            logger.error(
                "[Shogun] Connected provider '%s' has no concrete model ID configured.",
                provider.name,
            )
            yield f"data: {json.dumps({'type': 'error', 'content': 'The selected model provider has no valid model ID configured. Update it in The Katana.'})}\n\n"
            yield "data: [DONE]\n\n"
        ...
        # Operational short-term memory is deterministic context, separate
        # from semantic Archives recall, and is visible on every mission turn.
        from shogun.services.reminder_service import REMINDER_BOARD_GUIDE, ReminderService
        _reminder_board_context = "REMINDER BOARD — unresolved items: unavailable."
        try:
            async for board_db in get_db():
                _reminder_board_context = await ReminderService(board_db).prompt_context(
                    agent_id=agent.id,
                    user_id="local_user",
                )
            ...
            logger.debug("Reminder Board context skipped: %s", exc)
...
"""
        ...
        # ── 6. Build messages ─────────────────────────────────────────
        if classification and classification.get("_skill_context"):
            system_prompt += "\n\n" + classification["_skill_context"]
        system_prompt += "\n\n" + REMINDER_BOARD_GUIDE + "\n\n" + _reminder_board_context
        if _workflow_context:
            # Fixed system knowledge: never depend on probabilistic skill or
            # semantic-memory retrieval for workflow operations.
            system_prompt += "\n\n" + WORKFLOW_OPERATOR_GUIDE
        messages = [{"role": "system", "content": system_prompt}]
        for h in history[-10:]:
            if h.get("role") in ("user", "assistant") and h.get("content"):
        ...
        _operator_authorized_workflow_tools = operator_authorized_workflow_tools(user_msg)
        _policy_name = None
        async for db in get_db():
        ...
        if _operator_authorized_workflow_tools:
            messages[0]["content"] += (
                ...
                "- The operator's current message explicitly authorizes these medium-risk workflow "
                f"operations for this turn: {sorted(_operator_authorized_workflow_tools)}. "
                "Follow the mandatory discovery, inspection, mutation, and verification sequence.\n"
                "- Do not claim the tool is blocked or ask the operator to approve it elsewhere.\n"
            )
        if _workflow_confirmation_tools:
            messages[0]["content"] += (
                "\n\nAUTHORITATIVE WORKFLOW PERMISSION STATE:\n"
                "- These workflow tools are available through one-time operator confirmation because "
                f"their persistent permissions are disabled: {sorted(_workflow_confirmation_tools)}. "
                "Call the required tool normally; the backend will emit a real inline ToolGate card.\n"
        ...
        while True:
            ...
            apply_chat_reasoning(
                ...
                model_id=model_name,
                provider_config=provider.config,
                explicit_effort=_reasoning_effort,
            )
            ...
            if active_tools:
                ...
                # The first workflow operation is deterministic discovery. This
                # prevents a model from narrating that it "found" an object
                # without ever querying the workflow store.
                if (
                    _workflow_request
                    and not _any_tool_executed
                    and "list_agent_flows" in _active_tool_names
                ):
                    req_json["tool_choice"] = {
                        "type": "function",
                        "function": {"name": "list_agent_flows"},
                    }
                    ...
                    timeout=120.0,
                        ...
                        if resp.status_code >= 400:
                            ...
                            if not _retry_without_tools:
                                ...
                                try:
                                    await EL.emit_model_event(
                                        "model.error", f"LLM API returned HTTP {resp.status_code}",
                                    ...
                                    await EL.emit_incident_event(
                                        ...
                                        trace_id=_trace_id, agent_id=_agent_id_str,
                                        detail={"model": model_name, "provider": provider_name, "status_code": resp.status_code},
                                    )
                                except Exception:
                                    pass
                                yield f"data: {json.dumps({'type': 'error', 'content': f'Model provider returned HTTP {resp.status_code}. Check the Shogun logs.'})}\n\n"
                                yield "data: [DONE]\n\n"
                                return
            ...
            if not _any_tool_executed and not tool_calls_buffer and not _deduped_tc_matches and _active_tool_names:
                ...
                _force_nudge = (_small_model or _workflow_request) and len(_full_resp) > 20
                if (_mentioned or _force_nudge) and not _tool_retry_used:
                    ...
                    if not _mentioned:
                        if _workflow_request and "list_agent_flows" in _active_tool_names:
                            _mentioned = ["list_agent_flows"]
                        else:
            ...
            _tool_required = bool(_active_tool_names) and (
                _workflow_request
                or bool(classification and classification.get("task_type") in ("search", "skill", "agent"))
            )
        ...
        await _record_active_skill_outcomes(
            classification,
            "success",
            "Mission response delivered after orchestration and verification.",
        )
