...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ws = New-Object -ComObject WScript.Shell; $sc = $ws.CreateShortcut('%DESKTOP%\%SHORTCUT_NAME%.lnk'); $sc.TargetPath = '%TARGET%'; $sc.WorkingDirectory = '%SHOGUN_DIR%'; $sc.Description = 'Launch the Shogun AI Agent Framework'; $sc.WindowStyle = 1; $sc.IconLocation = '%ICON%,0'; $sc.Save()"
